...
Derived from `Crimson_Creed_Operations_System_PRD.pdf` (source of truth; the PDF
is not committed to this repository — ask the owner). This file records the
agreed architecture and phase breakdown. Update it as decisions change.
...
(app)/admin/activity
(app)/admin/audit
(app)/admin/relations     /new /[id]/edit
(app)/admin/fivem         live player monitor
...
| 17b ✅ | Submission order gate (`0045`): `create_order` refuses while a member owes a CONFIRMED submission for a closed month. Ships **off**; Super Admin toggle on `/admin/submissions`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | validate + build green                                          |
| 18 ✅  | Company stash stock types (`0037`), item images, relations (`0042–0044`, `0051`), text-length guards (`0046`), throttle GC (`0047`), atomic payroll finalize (`0049`), member dashboard RPC (`0050`), pinned search_path (`0052`), submission PIC (`0053`), one-step admin payment + paid-to (`0054–0055`).                                                                                                                                                                                                                                                                                                                                                                                                                                      | validate + build green                                          |
| 19 ✅  | Hardening pass: `admin_dashboard()` single-round-trip RPC (`0056`), SQL-side aggregates `cash_summary` / `my_earnings_summary` / `my_payslips` / `member_order_counts` (`0057`), composite list indexes (`0058`); stash list paginated in Postgres; auth rate limiting with trusted-IP detection + in-process fallback; unused dependencies / starter assets / dead helpers removed; SECURITY DEFINER authorization matrix + RLS visibility DB tests; unit + server-action tests.                                                                                                                                                                                                                                                                | validate + build green; 260 PGlite assertions; 105 Vitest tests |
